...
/** Gom latency theo service → p95/p99 từng nhóm. */
function summarizeLatenciesByGroup(results, key = 'service') {
  const groups = {};
  for (const r of results) {
    if (!r.httpOk || r.latencyMs == null) continue;
    const k = r[key] || 'unknown';
    if (!groups[k]) groups[k] = [];
    groups[k].push(r.latencyMs);
  }
  const out = {};
  for (const [k, samples] of Object.entries(groups)) {
    out[k] = summarizeLatencies(samples);
  }
  return out;
}

function scoreLatencyBudget(summary, budget) {
  if (!budget) return { pass: true, reasons: [] };
  const reasons = [];
  if (budget.maxP95Ms != null && summary.p95 != null && summary.p95 > budget.maxP95Ms) {
    reasons.push(`p95 ${summary.p95}ms > max ${budget.maxP95Ms}ms`);
  }
  if (budget.maxP99Ms != null && summary.p99 != null && summary.p99 > budget.maxP99Ms) {
    reasons.push(`p99 ${summary.p99}ms > max ${budget.maxP99Ms}ms`);
  }
